...
[self.config]
  Option description from self.config.json
...
class WFCatalogCollector():
    ...
    def _setOptions(self):
        ...
        nInput = 6 - [self.args['date'], self.args['file'], self.args['dir'],
                      self.args['list'], self.args['past'], self.args['glob']].count(None)
        if nInput == 0:
        ...
        if not self.config['MONGO']['ENABLED'] and self.args['update']:
            raise Exception(
                "Cannot update files when database connection is disabled")
    ...
    def _collectFilesFromDate(self, date):
        ...
        if self.config['STRUCTURE'] == 'ODC':
            ...
            collectedFiles = [os.path.join(directory, f) for f in os.listdir(
    ...
    def _getFiles(self):
        ...
        if self.args['past']:
            ...
            self.log.info("Collected %d file(s) from the past %s" %
                          (len(self.files), self.args['past']))
        ...
        # Raise if an invalid input directory is given
            ...
            self.files = [os.path.join(root, f) for root, dirs, files in os.walk(
                self.args['dir']) for f in files if os.path.isfile(os.path.join(root, f))]
            self.log.info("Collected %d file(s) from directory %s" %
                          (len(self.files), self.args['dir']))
        ...
        # Specific date as input (with optional range)
            ...
            for day in range(abs(n_days)):
                if n_days > 0:
                    ...
                    self.files += self._collectFilesFromDate(specific_date - datetime.timedelta(days=day))
            self.log.info("Collected %d file(s) from date %s +%d days" %
                          (len(self.files), self.args['date'], n_days))
    ...
    def _filterFiles(self):
        ...
        self.log.info("Discovered %d new file(s) for processing" %
                      (len(new_files)))

        # If we are updating, remove old documents and add changed document to
        # the process list
        if self.args['update']:
            changed_files = self._getChangedFiles()
            if self.args['force']:
                self.log.info("Forcing update of %d file(s) in database" %
                              (len(changed_files)))
            else:
                self.log.info("Discovered %d file(s) with changed checksum in database" %
                              (len(changed_files)))
        else:
            changed_files = []
        ...
        for file in self.files:
            ...
            for document in self.mongo.getDailyFilesById(file):
                ...
                for used_files in document['files']:
                    ...
                    self.log.info("Comparing MD5checksums for %s" %
                                  used_files['name'])
                    ...
                    if MD5Hash != used_files['chksm']:
                        ...
                        self.log.info("Adding file %s for updating" %
                                      document["fileId"])
                        changedFiles.append(document["fileId"])
    ...
    def _callObsPyMetadata(self, files, start, end, granule):
        ...
        try:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                if granule == 'daily':
                    metadata = MSEEDMetadata(files, starttime=start, endtime=end, 
                                             add_flags=self.args['flags'],
    ...
    def collectMetadata(self, file):
        ...
        if not os.path.isfile(file):
            self.log.info("File no longer exists in archive %s" %
        ...
        try:
            ...
            daily_meta = self._callObsPyMetadata(fas['files'], granule['start'],
                                                 granule['end'], 'daily')
            daily_meta.update({'fileId': os.path.basename(file)})
        except Exception as ex:
            self.log.error("Could not get daily metadata for %s" %
                           os.path.basename(file))
            self.log.error(ex)
            return
        ...
        for granule in fas['segments']['hourly']:
            try:
                hourly_meta = self._callObsPyMetadata(fas['files'], granule['start'],
                                                      granule['end'], 'hourly')
    ...
    def _storeOutput(self, documents):
        ...
        if not self._isNewDocument(documents['daily']['fileId']):
            self.log.error("Stop: document with this id is already in the database: %s" %
                           documents['daily']['fileId'])
            return
        ...
        if self.args['hourly']:
            for i, granule in enumerate(documents['hourly']):
                try:
                    ...
                    self.log.info("[%d/%d] Succesfully stored hourly granule" %
                                  (i + 1, len(documents['hourly'])))
                except Exception as ex:
        ...
        if self.args['csegs'] and not qc_metadata_daily['cont']:
            ...
            self.log.info("Succesfully stored %d continuous segment(s) to database" % 
                          len(documents['daily']['c_segments']))
    ...
    def _getFileChecksums(self, files):
        ...
        if self.config['ENABLE_DUBLIN_CORE']:
    ...
    def _createDataObject(self, file):
        ...
        document = {
            ...
            'dc:creator': self.config['ARCHIVE'],
            'dc:contributor': 'network operator',
            'dc:publisher': self.config['ARCHIVE'],
    ...
    def _getFlagKeys(self, trace, flag_type):
        ...
        if flag_type == 'activity_flags':
            ...
            raise Exception(
                "Unknown flag type in mSEEDMetadataCollector._getFlagKeys")
    ...
    def _getStatsObject(self, file):
        ...
        if self.config['STRUCTURE'] == 'ODC':
            ...
            raise Exception(
                "Unknown directory structure in mSEEDMetadataCollector._getStatsObject")
    ...
    def _getFilename(self, stats):
        ...
        if self.config['STRUCTURE'] == 'ODC':
            filename = ".".join([stats['station'], stats['channel'], stats['network'],
                                 stats['year'], stats['jday']])

        elif self.config['STRUCTURE'] == 'SDS':
            filename = ".".join([stats['network'], stats['station'], stats['location'],
                                 stats['channel'], stats['dtype'], stats['year'], stats['jday']])
    ...
    def _getFileDirectory(self, stats):
        ...
        if self.config['STRUCTURE'] == 'ODC':
            filepath = os.path.join(stats['year'], stats['jday'], 
                                    self._getFilename(stats))

        elif self.config['STRUCTURE'] == 'SDS':
            filepath = os.path.join(stats['year'], stats['network'], stats['station'],
                                    stats['channel'] + "." + stats['dtype'],
                                    self._getFilename(stats))
    ...
    def _collectFilesAndSegments(self, file):
        ...
        self.log.info("[%d/%d] File %s prepared with %s" % (self.file_counter, self.totalFiles,
                                                            os.path.basename(file),
                                                            [os.path.basename(f) for f in day_files]))
